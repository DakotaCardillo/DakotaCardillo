<script lang="ts">
	...
	const highlights = [
		'Directed multi‑disciplinary teams delivering large‑scale UE5 simulations.',
	...
	const experience = [
		...
		period: '2023 — Present',
		bullets: [
			'Led new UE5 feature development and refined server & project architecture.',
			'Tripled Perforce/Zen throughput with multithreaded sync & modular workspaces.',
			'Coordinated delivery across engineers, artists, and QA to hit aggressive milestones.',
			'Presented testing strategies tailored to real‑time simulation pipelines.'
		]
		},
		{
		...
		period: '2020 — 2023',
		bullets: [
			'Maintained C# backend services powering complex SQL workflows for reports.',
			'Co‑designed tax ID encryption to track taxable accounts without exposing PII.',
			'Built product knowledge across React/Redux & data pipelines (from zero).'
		]
		},
		{
		...
		period: '2019 — 2020',
		bullets: [
			'Led development on 737/P‑8A & AH‑6i virtual maintenance trainers (Unity).',
			'Created automated UI regression suite to record & validate discrepancies.',
			'Shipped tools that removed bottlenecks for artists & engineers (prefabs, search).'
		]
		},
		{
		...
		period: '2016 — 2019',
		bullets: [
			'Authored procedural animation for realistic flight‑control mechanisms.',
			'Built a low‑latency VNC streaming layer inside Unity for remote trainers.',
				  ...
				  <div>
					...
					<div class="flex flex-wrap gap-2">
					  {#each group.items as s}
						<span class="rounded-lg border border-zinc-700 bg-surface px-3 py-1.5 text-sm text-zinc-300">{s}</span>
					  {/each}
					</div>
				  </div>
				{/each}
				...
				{#each experience as job}
				  <article class="timeline-item">
					...
					<ul class="mt-2 space-y-2">
					  {#each (compact ? job.bullets.slice(0, 3) : job.bullets) as b}
				...
				{#each projects as p}
				  <div class="rounded-2xl border border-zinc-700 bg-[--surface-2] p-4">
					<div class="flex items-center justify-between">
					  ...
					  <span class="text-xs px-2 py-1 rounded bg-accent/10 text-accent border border-accent/30">{p.year}</span>
					</div>
			...
			<div class="card p-6 md:p-8">
			  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div>
				  ...
				  <p class="text-zinc-300 mt-1">Unreal Engine • Simulation • Tools • High‑performance systems</p>
				</div>
