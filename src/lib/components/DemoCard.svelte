<script lang="ts">
    ...
    const onIntersect = (entries: IntersectionObserverEntry[]) => {
      if (entries[0].isIntersecting) { visible = true; observer?.disconnect(); }
    };
    $effect(() => {
    ...
    <div class="aspect-video bg-zinc-900">
      {#if visible}
        <video class="h-full w-full object-cover" autoplay muted loop playsinline poster={d.media.poster}>
          {#if d.media.webm}<source src={d.media.webm} type="video/webm" />{/if}
          {#if d.media.mp4}<source src={d.media.mp4} type="video/mp4" />{/if}
        </video>
      {/if}
    </div>
    <div class="p-4">
      <div class="flex items-center justify-between">
        ...
        {#if d.year}<span class="text-xs px-2 py-1 rounded bg-accent/10 text-accent border border-accent/30">{d.year}</span>{/if}
      </div>
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        {#if d.links.live}<a class="btn" href={d.links.live} target="_blank">Live</a>{/if}
